    ...
    while (count <= 6){
      ...
      if (Firebase.RTDB.getInt(&fbdo, path)) {
        if (fbdo.dataType() == "int") {
          ...
          if (intSet != 0){
            digitalWrite(rgb[count-4][0], HIGH);
            digitalWrite(rgb[count-4][1], HIGH);
            digitalWrite(rgb[count-4][2], LOW);
          }
          else if(intStatus == 255){
            digitalWrite(rgb[count-4][0], LOW);
            digitalWrite(rgb[count-4][1], HIGH);
            digitalWrite(rgb[count-4][2], HIGH);
          }
          else {
            digitalWrite(rgb[count-4][0], HIGH);
            digitalWrite(rgb[count-4][1], LOW);
            digitalWrite(rgb[count-4][2], HIGH);
          }
        }
      }
